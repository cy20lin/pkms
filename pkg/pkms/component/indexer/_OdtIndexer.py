...
from pkms.lib.odt_to_html import (
    OdtToHtmlConverter,
    OdtToHtmlConverterConfig,
    OdtToHtmlConverterRuntime,
)
from bs4 import BeautifulSoup

class OdtIndexerConfig(IndexerConfig):
    type: Literal['OdtIndexerConfig'] = 'OdtIndexerConfig'
    converter: OdtToHtmlConverterConfig

class OdtIndexerRuntime(IndexerRuntime):
    def __init__(self, *args, config: OdtIndexerConfig, **kwargs):
        self.converter = OdtToHtmlConverterRuntime(config=config.converter)

class OdtIndexer(Indexer):
    Config = OdtIndexerConfig
    Runtime = OdtIndexerRuntime

    def __init__(self, config: OdtIndexerConfig, runtime: Optional[OdtIndexerRuntime] = None):
        if runtime is None:
            runtime = OdtIndexerRuntime(config=config)
        super().__init__(config=config, runtime=runtime)
        self.converter = OdtToHtmlConverter(config=config.converter, runtime=runtime.converter)
