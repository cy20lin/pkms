...
class HtmlIndexer(Indexer):
    ...
    def index(self, file_location: FileLocation) -> IndexedDocument:
        ...
        file_name_parsed = parse_file_name(file_path_)
        file_size = file_path_.stat().st_size
        file_name = file_name_parsed["name"]
        file_extension = file_name_parsed["extension"]
        file_name_id = file_name_parsed["id"]
        file_name_title = file_name_parsed["title"]
        file_name_importance = file_name_parsed["importance"]
        ...
        title = html_metadata.get('title',None)
        if not title: title = file_name_title

        # Set saved_datetime, fallback if needed
        if isinstance(sf_metadata,dict) and 'saved_date' in sf_metadata:
            saved_datetime = sf_metadata['saved_date']
        else:
            saved_datetime = datetime.datetime.fromtimestamp(file_path_.stat().st_ctime).astimezone().isoformat()
        ...
        index = IndexedDocument(**{
            ...
            "file_kind": 'snapshot',
            "title": title,
            "importance": file_name_importance,
            "origin_uri": sf_metadata.get("url", None) if sf_metadata else None,
            "text": text,
            "extra": {
                "html": html_metadata,
