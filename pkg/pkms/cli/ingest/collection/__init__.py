...
def parse_args(argv:list[str]=None):
    parser = argparse.ArgumentParser(
        "pkms.cli.ingest.collection",
        description="Ingest specified collection"
    )
    parser.add_argument("collection", help='Path to collection dir')
...
def main(argv):
    ...
    collection_path = pathlib.Path(args.collection).absolute().as_posix()
    collection_location = FileLocation.from_filesystem_path(
        collection_path, path_convention=path_convention
    )
    capability.ingest_collection(collection_location, dry_run=args.dry_run)
